...
const imageName = process.env.BACKEND_DOCKER_SMOKE_IMAGE ?? 'ai-fitness-coach-backend:smoke'
const containerName =
  process.env.BACKEND_DOCKER_SMOKE_CONTAINER ??
  `ai-fitness-coach-backend-smoke-${repositoryHash}-${process.pid}`
...
const databaseUrlForContainer =
  ...
  'postgresql://superuser:superpassword@postgres_test:5432/ai_fitness_coach_test?schema=public'
