...
| AI fitness domain               | absent   | Требуется для первого пилота. Реализованы ручные срезы дневника питания/активности/замеров, подтверждение полноты дня, история выполненных тренировок и fail-closed граница каталога упражнений. Первый активный технический образец — утверждённая карточка `bodyweight-squat` с локальным видео; остальные карточки скрыты. Evidence-approved расчёты, полноценная утверждённая библиотека, назначаемые планы, AI/voice/photo input и остальной mobile UX ещё отсутствуют. |
| Hybrid onboarding               | absent   | Обязателен в первом пилоте. Ручной authenticated backend flow, contracts, state rules, PostgreSQL persistence и мобильный структурированный путь существуют. Текстовый AI-путь, голос, безопасное восстановление несинхронизированного локального черновика и полная пилотная проверка ещё отсутствуют, поэтому capability не активирован.                                                                                                                                              |
| Persistent user memory          | absent   | Обязательна в первом пилоте. Backend сохраняет подтверждённый профиль, цели, замеры, safety, версии планов, provenance, ручные записи питания/активности, подтверждение полноты дня и структурированную историю выполненных тренировок; поддерживает исправление, удаление, изоляцию аккаунтов и bounded onboarding context. Полный профильный UX, AI coach context и сквозное обновление всех производных данных ещё отсутствуют; foundation не активирует capability. |
| Today dashboard                 | absent   | Мобильный каркас реализует пять вкладок, верхнюю плашку, переход в «Прогресс», быстрое открытие дневника и честное отображение подтверждённой полноты дня. Плашка показывает только явно записанные калории; расход, цель, баланс и цвет остаются недоступны без evidence-approved расчёта. План дня и контекстные обращения ещё отсутствуют, поэтому capability не активирован.                                                                                                                     |
...
| Nutrition and food-photo input  | absent   | Ручная подтверждённая запись еды с необязательными известными КБЖУ реализована в первом срезе дневника. Оценка по описанию, фото-ввод, распознавание и подтверждение AI-оценки ещё отсутствуют; аватарный upload не является распознаванием еды, поэтому capability остаётся `absent`.                                                                                                                                                                                                       |
| Training plans and history      | absent   | Реализована ручная подтверждённая история выполненных тренировок и версионируемая граница каталога. Authenticated API не выдаёт черновики и active-карточки без инструкции, review reference и финальной движущейся демонстрации. В [`docs/EXERCISE_LIBRARY.md`](docs/EXERCISE_LIBRARY.md) русские тексты и статичные раскадровки версии 1 для всех 21 карточки утверждены владельцем-тренером; `bodyweight-squat` дополнительно имеет утверждённые стартовый кадр и короткое видео и доступен как первый локальный технический образец. Остальные 20 движущихся демонстраций, назначаемый четырёхнедельный план, выполнение плановой тренировки, offline sync и прогрессия ещё отсутствуют, поэтому capability остаётся `absent`. |
| Measurements and weekly review  | absent   | Ручные добровольные записи веса, стандартных и собственных замеров и подтверждение полноты дня реализованы с датой, источником, исправлением, удалением и изоляцией аккаунтов. «Прогресс» загружает месячную историю текущего пользователя, нейтрально отмечает подтверждённые дни и показывает фактическую сводку выбранной даты без преждевременной энергетической оценки. Графики/тенденции, сопоставимость, энергетические статусы и недельный разбор ещё отсутствуют, поэтому capability остаётся `absent`. |
