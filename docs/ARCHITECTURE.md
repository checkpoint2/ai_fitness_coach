...
- Zod schemas come from `@ai-fitness-coach/contracts`.
...
Local PostgreSQL is provided by Docker Compose, not by a native database install. The development service uses `postgres:18-alpine`, exposes `ai_fitness_coach` on host port `54329`, and stores data in the `postgres_18_data` volume. The test service uses the same image with database `ai_fitness_coach_test`; automated runners set `POSTGRES_TEST_PORT` to a repository-derived port when they need isolation. PostgreSQL 18 is intentional here because the backend schema relies on the native `uuidv7()` database function.
